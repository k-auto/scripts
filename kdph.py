...
def mkpkg(folder, key=None, token=None):
    ...
    try:
        ...
        pass
    package_archive = f"{folder_name}.tar.gz"
    ...
    _archive_folder(folder_path, package_archive)
    encrypt_file(package_archive, package_enc, key)
    package_cluster = _cluster_file(package_enc)
    _github_upload(token, folder_name, pkg_docs, "Knexyce Package documentation manifested.")
    _github_upload(token, folder_name, package_cluster, "Knexyce Package manifested.")
    _github_upload(token, folder_name, kdph_local, "KDPH manifested.", ["knexyce", "kdph", "secure", "cryptography"], "Knexyce Packages are securely encrypted archives of data managed by KDPH.")
